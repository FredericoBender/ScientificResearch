...
public class Bull {
   ...
   * It launches the algorithm
     * @return 
   */
  public ArrayList<Double>  execute(int FM) {
    ArrayList<Double> vetorFinal = new ArrayList<>();
    if (this.somethingWrong) { //We do not execute the program
	  ...
	  this.ruleBase = new RuleBase(this.dataBase, this.train, this.K, this.typeInference, FM, this.opFM);
	  this.apriori = new Apriori(this.ruleBase, this.dataBase, this.train, this.minsup, this.minconf, this.depth, FM, this.opFM);
	  ...
	  this.ruleBase = pop.getBestRB();
          ...
          System.out.println("\n Inference["+this.typeInference+"];  Measure["+FM+"];  Aggregation["+this.opFM+"]\n");
          System.out.println("TRAINING             |            TESTING");
          String result = (""+this.ruleBase.evaluate(this.val)+"\t"+this.ruleBase.evaluate(this.test));
          
          double resultadoTraining = this.ruleBase.evaluate(this.val);
          double resultadoTesting = this.ruleBase.evaluate(this.test);
          vetorFinal.add(resultadoTraining);
          vetorFinal.add(resultadoTesting);
